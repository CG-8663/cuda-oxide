...
1. **Renames** the function to `cuda_oxide_kernel_<name>` so the compiler's
   collector can identify it as a device entry point.
...
1. Scan all compilation units for functions with the `cuda_oxide_kernel_` prefix
   (generated by `#[kernel]`).
