    ...
    unsafe {
        ...
        while k_idx < k_iters {
            ...
            if is_thread0 {
                ...
                cp_async_bulk_tensor_2d_g2s(smem_a_ptr, a_tma, k_base, m_offset, &raw mut TMA_BAR);
                cp_async_bulk_tensor_2d_g2s(smem_b_ptr, b_tma, k_base, n_offset, &raw mut TMA_BAR);
                mbarrier_arrive_expect_tx(&raw const TMA_BAR, 1, A_TILE_BYTES + B_TILE_BYTES);
            }
        ...
        while k_idx < k_iters {
            ...
            if is_thread0 && k_idx + 1 < k_iters {
                ...
                if buf == 0 {
                    ...
                    mbarrier_arrive_expect_tx(&raw const TMA_BAR1, 1, A_TILE_BYTES + B_TILE_BYTES);
                } else {
                    cp_async_bulk_tensor_2d_g2s(
                        &raw mut SMEM_A0 as *mut u8,
                        a_tma,
                ...
                while tmem_row_block < 2 {
                    ...
                    while col_block < 8 {
                        ...
                        let regs_a = tcgen05_ld_16x256b_pure(
                            tmem_addr + tmem_stage_offset + (tmem_row << 16) + col_offset as u32,
                        );
                        tcgen05_load_wait();
                ...
                while k_idx < k_iters {
                    ...
                    if elect_one_cta {
                        ...
                        if is_lane0 {
                            ...
                            tcgen05_commit_multicast_cg2(mma_bar_mut as *mut u64, CTA_MASK_PAIR);
                        }
                    }
...
fn run_correctness_test(
    ...
    cuda_launch! {
        kernel: gemm_sol_tiled,
        stream: stream,
        module: module,
        config: cfg,
        args: [a_tma_ptr, b_tma_ptr, slice_mut(dev_output), n_arg, k_arg]
    }?;
...
fn run_correctness_test_persistent(
    ...
    cuda_launch! {
        kernel: gemm_sol_persistent,
        stream: stream,
        module: module,
        config: cfg,
        cluster_dim: (4, 1, 1),
        args: [a_tma_ptr, b_tma_ptr, slice_mut(dev_output), counter_ptr, n_arg, k_arg, tiles_m, tiles_n]
    }?;
...
fn run_correctness_test_clc_multicast_4_stage_pipeline(
    ...
    cuda_launch! {
        kernel: gemm_sol_clc_multicast_4_stage_pipeline,
        stream: stream,
        module: module,
        config: cfg,
        cluster_dim: (2, 1, 1),
