...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // u32 div: 0xFFFF_FFFE / 2 = 0x7FFF_FFFF
    {
        ...
        cuda_launch! {
            kernel: test_u32_div_msb,
            stream: stream, module: module, config: cfg,
            args: [0xFFFF_FFFEu32, 2u32, slice_mut(out)]
        }?;
    ...
    // u32 rem: 0xFFFF_FFFF % 10 = 5
    {
        let mut out = DeviceBuffer::<u32>::zeroed(&stream, N)?;
        cuda_launch! {
            kernel: test_u32_rem_msb,
            stream: stream, module: module, config: cfg,
            args: [0xFFFF_FFFFu32, 10u32, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == 5u32 {
    ...
    // u64 div: 0x8000_0000_0000_0000 / 2 = 0x4000_0000_0000_0000
    {
        ...
        cuda_launch! {
            kernel: test_u64_div_msb,
            stream: stream, module: module, config: cfg,
            args: [0x8000_0000_0000_0000u64, 2u64, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
    ...
    // u64 rem: u64::MAX % 7 = 1
    {
        let mut out = DeviceBuffer::<u64>::zeroed(&stream, N)?;
        cuda_launch! {
            kernel: test_u64_rem_msb,
            stream: stream, module: module, config: cfg,
            args: [u64::MAX, 7u64, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == 1u64 {
    ...
    // u32 gt: 0x8000_0000 > 0x7FFF_FFFF = true (1)
    {
        let mut out = DeviceBuffer::<u32>::zeroed(&stream, N)?;
        cuda_launch! {
            kernel: test_u32_cmp_gt_msb,
            stream: stream, module: module, config: cfg,
            args: [0x8000_0000u32, 0x7FFF_FFFFu32, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == 1u32 {
    ...
    // u64 gt: u64::MAX > 0 = true (1)
    {
        let mut out = DeviceBuffer::<u32>::zeroed(&stream, N)?;
        cuda_launch! {
            kernel: test_u64_cmp_gt_msb,
            stream: stream, module: module, config: cfg,
            args: [u64::MAX, 0u64, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == 1u32 {
    ...
    // u32 lt: 0x7FFF_FFFF < 0x8000_0000 = true (1)
    {
        let mut out = DeviceBuffer::<u32>::zeroed(&stream, N)?;
        cuda_launch! {
            kernel: test_u32_cmp_lt_msb,
            stream: stream, module: module, config: cfg,
            args: [0x7FFF_FFFFu32, 0x8000_0000u32, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == 1u32 {
    ...
    // i32 shr: -8 >> 1 = -4
    {
        ...
        cuda_launch! {
            kernel: test_i32_shr_negative,
            stream: stream, module: module, config: cfg,
            args: [-8i32, 1u32, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == -4i32 {
    ...
    // i64 shr: -1024 >> 3 = -128
    {
        ...
        cuda_launch! {
            kernel: test_i64_shr_negative,
            stream: stream, module: module, config: cfg,
            args: [-1024i64, 3u32, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == -128i64 {
    ...
    // signed: -7 / 2 = -3; unsigned: 0xFFFF_FFFE / 2 = 0x7FFF_FFFF
    {
        ...
        cuda_launch! {
            kernel: test_mixed_div,
            stream: stream, module: module, config: cfg,
            args: [-7i32, 2i32, 0xFFFF_FFFEu32, 2u32, slice_mut(out_signed), slice_mut(out_unsigned)]
        }?;
    ...
    // u32 wrapping add: MAX + 1 = 0
    {
        let mut out = DeviceBuffer::<u32>::zeroed(&stream, N)?;
        cuda_launch! {
            kernel: test_u32_wrapping_add,
            stream: stream, module: module, config: cfg,
            args: [u32::MAX, 1u32, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == 0u32 {
    ...
    // i32 wrapping mul: i32::MAX * 2 = -2
    {
        let mut out = DeviceBuffer::<i32>::zeroed(&stream, N)?;
        cuda_launch! {
            kernel: test_i32_wrapping_mul,
            stream: stream, module: module, config: cfg,
            args: [i32::MAX, 2i32, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == -2i32 {
    ...
    // widening chain: u8(0xFF) -> u32 -> u64 = 255
    {
        let mut out = DeviceBuffer::<u64>::zeroed(&stream, N)?;
        cuda_launch! {
            kernel: test_widening_chain,
            stream: stream, module: module, config: cfg,
            args: [0xFFu8, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == 255u64 {
    ...
    // narrowing signed: i64(-42) -> i32 = -42
    {
        let mut out = DeviceBuffer::<i32>::zeroed(&stream, N)?;
        cuda_launch! {
            kernel: test_narrowing_signed,
            stream: stream, module: module, config: cfg,
            args: [-42i64, slice_mut(out)]
        }?;
        let r = out.to_host_vec(&stream)?;
        if r[0] == -42i32 {
